...
{
    partial class LubricatingTool : RepairTool
    {
        //how much lubrication is dealt per delta time
        private float lubricatingoil;
        ...
        public float Lubricatingoil
        {
            get { return lubricatingoil; }
            set { lubricatingoil = value; }
        }

        public LubricatingTool(Item item, ContentXElement element)
            : base(item, element)
        {
        ...
        private void Repair(Vector2 rayStart, Vector2 rayEnd, float deltaTime, Character user, float degreeOfSuccess, List<Body> ignoredBodies)
        {
            ...
            if (lubricatingoil > 0.0f && item.Submarine != null)
            {
                    ...
                    if (it.Submarine == item.Submarine && it.GetComponent<InternalCombustionEngine>() is { } engine)
                    {
                        ...
                        if (collisionRect.Left < pos.X && collisionRect.Right > pos.X && collisionRect.Bottom > pos.Y &&  collisionRect.Top < pos.Y) // It seems that the top and bottom are reversed
                        {
                            ...
                            if (collision == null)
                            {
                                ...
                                if (engine.lubricant < engine.lubricantMax)
                                {
                                    engine.lubricant += lubricatingoil * deltaTime;
                                }
                                ...
                                user?.UpdateHUDProgressBar(engine, engine.Item.DrawPosition + new Vector2(barOffset, 0), engine.lubricant / engine.lubricantMax, GUIStyle.Yellow, GUIStyle.Yellow, "progressbar.Lubricating");
#endif
                            }
                        }
                    }
